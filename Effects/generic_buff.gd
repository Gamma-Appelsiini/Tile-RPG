...
@onready var balls: CPUParticles3D = $Balls
@onready var aura_mesh: MeshInstance3D = $AuraMesh

const LIFE_TIME:float = 0.8
...
func set_color(new_color:Color = Color(1.0, 1.0, 1.0, 1.0)) -> void:
	...
	shader_material.set_shader_parameter("color_parameter", start_color)

func animate_aura_color() -> void:
	var shader_material:ShaderMaterial = aura_mesh.material_override
	
	var tween:Tween = create_tween().set_ease(Tween.EASE_OUT)
	tween.tween_property(shader_material, "shader_parameter/color_parameter", end_color, LIFE_TIME / 2)
	
	await tween.finished
	tween = create_tween().set_ease(Tween.EASE_IN)
	tween.tween_property(shader_material, "shader_parameter/color_parameter", start_color, LIFE_TIME / 2)
